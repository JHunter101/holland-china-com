	...
	const teamMembers = derived(langData, ($langData) => {
		...
		while ($langData[`SEAN_01_about-team-member_name-${i}`]) {
			list.push({
				...
				bio: $langData[`SEAN_01_about-team-member_bio-${i}`] || '',
				img: $langData[`SEAN_01_about-team-member_img-${i}`] || 'https://placehold.co/400x400'
			});
			i++;
		}
		return list;
...
<section class="screen-tuck flex-natural text--justify flex-col">
	<h1 class="subtitle text-large">{$langData['HCC_01_about-main-desc-1']}</h1>
...
<!-- Team Members -->
<section class="screen-tuck flex-natural flex-col">
	<h1 class="subtitle text-large">{$langData['SEAN_01_about-main-title'] || 'Our Team'}</h1>
